...
./ ADD NAME=INSTALL
PROC 1 SOFT
/* Generates JCL used to install software with the install command */
/* LICENSE GPLv2 AUTHOR PHILIP YOUNG */
CONTROL NOMSG
FREE ATTR(TEMPATTR)
FREE FI(TEMPDSN)
FREE DA('&SYSUID..TEMPINST')
ATTR TEMPATTR RECFM(F B) LRECL(80) BLKSIZE(3120)
ALLOC FI(TEMPDSN) DA('&SYSUID..TEMPINST') +
NEW USING(TEMPATTR) UNIT(VIO) CATALOG
OPENFILE TEMPDSN OUTPUT
SET &TEMPDSN = &STR(//INSTALL  JOB (JOB),'INSTALL',CLASS=A,MSGCLASS=A)
PUTFILE TEMPDSN
SET &TEMPDSN = &STR(//RUNCMD   EXEC PGM=MDDIAG8,)
PUTFILE TEMPDSN
SET &TEMPDSN = &STR(//   PARM='script ../SOFTWARE/&SOFT/install.rc')
PUTFILE TEMPDSN
CLOSFILE TEMPDSN
SUBMIT '&SYSUID..TEMPINST'
FREE DA('&SYSUID..TEMPINST') FI(TEMPDSN)
DELETE '&SYSUID..TEMPINST'
CONTROL MSG
WRITE *** INSTALL FOR &SOFT SUBMITTED
@@